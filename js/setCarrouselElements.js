...
fetch(`${url}/products/all`)
    ...
    .then((data) => {
        ...
        products.forEach(prod => {
            ...
            let minCurrencyPrice = (prod.prices?.min.currencyCode == 'EUR') ? '€' : '$';
            let maxCurrencyPrice = (prod.prices?.max.currencyCode == 'EUR') ? '€' : '$';

            
            // discountEl.innerText = '20%';
            discountEl.innerText = (getDiscount(minPrice, maxPrice)) ? getDiscount(minPrice, maxPrice) + '%' : '';

            minPrice = minCurrencyPrice + '' + minPrice;
            maxPrice = maxCurrencyPrice + '' + maxPrice;
